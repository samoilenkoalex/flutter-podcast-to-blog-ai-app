...
final enclosureTypeValues = EnumValues({
  "audio/mpeg": EnclosureType.AUDIO_MPEG
});
...
final feedLanguageValues = EnumValues({
  "en": FeedLanguage.EN
});
...
class Value {
  ...
  factory Value.fromMap(Map<String, dynamic> json) => Value(
    ...
    destinations: json["destinations"] != null
        ? List<Destination>.from(json["destinations"].map((x) => Destination.fromMap(x)))
        : null,
  );
...
enum CustomValue {
  J_NH_G48_K_DP_K6_UH6_AJMHH_J,
  THE_01_IM_QKT4_B_FZ_AI_SYNXC_Q_QQD,
  X3_VX_ZTBCF_IBVLI_UQZ_WKV
}
...
enum Name {
  BOOSTBOT_FOUNTAIN_FM,
  JOHNS_CREEK_STUDIOS,
  OP3_AND_REFLEX_LIVEWIRE_IO,
  PODCASTINDEX_ORG,
  RANDY_BLACK,
  RSS_BLUE
}
...
final destinationTypeValues = EnumValues({
